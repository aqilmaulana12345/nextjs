...
module.exports = {
  ...
  detailPage: async (req, res) => {
    try {
      ...
      const payments = await Payment.find({ status: 'Y' }).populate('banks').select('banks _id type')
      
      res.status(200).json({ data: {detail: voucher, payment : payments} })
    ...
    try {
      ...
      const payload = {
        historyVoucherTopup: {
          ...
          idItemVoca: res_nominal._doc.idItemVoca,
          idCategoryVoca: res_voucher._doc.category.idCategoryVoca,
